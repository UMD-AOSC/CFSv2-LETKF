...
public  diag_manager_init, send_data, send_tile_averaged_data, diag_manager_end,  &
        register_diag_field, register_static_field,get_date_dif, &
        diag_axis_init, get_base_time, get_base_date, need_data, average_tiles
public :: DIAG_ALL,DIAG_OCEAN,DIAG_OTHER,DIAG_SECONDS,DIAG_MINUTES,DIAG_HOURS, & 
...
! test file format to make sure its OK
   ...
   if(output_freq_units == DIAG_MONTHS .or. new_file_freq_units == DIAG_MONTHS) then
      if (base_day >= 29 .and. get_calendar_type() /= GREGORIAN .and. mpp_pe() == mpp_root_pe()) &
           call error_mesg('diag_manager_init','USER BEWARE: day>=29 and output_frequency= ' &
           //'MONTHS may lead to FATAL error in set_date for some months',WARNING)
   endif
85 continue
enddo
