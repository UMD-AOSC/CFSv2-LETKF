...
!-->cpl test
        if( .NOT.any(pelist.EQ.pe) )then
          write( stderr(),* )'pe=', pe, ' pelist=', pelist
          call mpp_error( FATAL, 'MPP_DEFINE_DOMAINS2D: pelist from outside,pe must be in pelist.' )
        endif
!<-- cp test
    else
       allocate( pes(0:mpp_npes()-1) )
       ...
       !          pes = (/ (i,i=0,mpp_npes()-1) /)
!-->cpl test
        if( .NOT.any(pes.EQ.pe) )then
          write( stderr(),* )'pe=', pe, ' pelist=', pes
          call mpp_error( FATAL, 'MPP_DEFINE_DOMAINS2D: pelist from get_current_pelist, pe must be in pelist.' )
        endif
!<-- cp test
     endif

!    print *,'in mpp_define_domains2D,pelist=',pes,'ndivx=',ndivx,'ndivy=',ndivy
