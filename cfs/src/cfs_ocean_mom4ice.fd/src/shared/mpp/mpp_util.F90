...
!-->cpl insertion
     call MPI_COMM_RANK(peset(current_peset_num)%id,pe,ierr)
!<-- cpl insertion
    mpp_pe = pe
    return
  end function mpp_pe
...
!calls mld_id from threadloc.c on sgi, which returns the hardware node ID from /hw/nodenum/...
  ...
  subroutine mpp_set_current_pelist( pelist, no_sync,pesetnum )
...
!       if( .NOT.ANY(pe.EQ.pelist) )call mpp_error( FATAL, 'MPP_SET_CURRENT_PELIST: pe must be in pelist.' )
       ...
       current_peset_num=1
!--> cpl insertion never set pelist for comm_WORLD,set them to ocean_comm
...
!-->cpl insertion
    if( current_peset_num .gt.1 ) then
      call MPI_COMM_RANK(peset(current_peset_num)%id,pe,ierr)
    else
      call MPI_COMM_RANK(peset(world_peset_num)%id,pe,ierr)
    endif
!<--cpl insertion
!    print *,'in mpp_set_current_pelist, current_peset_num=', current_peset_num
!    if(present(pelist)) print *,'in mpp_set_current_pelist, mype=',pe, 'pelist=', pelist
