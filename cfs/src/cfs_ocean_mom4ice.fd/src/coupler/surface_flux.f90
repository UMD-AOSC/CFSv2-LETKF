...
use   monin_obukhov_mod, only: mo_drag, mo_drag_cpl, mo_profile
...
! check err
  do i=1,size(t_atm(:))
     if (t_surf0(i) .LT. 150.0) then
         print *,'WARNING: t_surf0 is too low at #:',t_surf0(i),i
         t_surf0(i)=150.
         print *,'reset t_surf0 to 150.0'
     endif
     if (t_surf1(i) .GT. 350.0) then
         print *,'WARNING: t_surf1 is too high at #:',t_surf1(i),i
         t_surf1(i)=350.
         print *,'reset t_surf1 to 350.0'
     endif
  enddo
!
...
!     thv_surf= t_surf0                        ! surface virtual (potential) T -- just for testing tun off the q_surf
  ...
  if (gfs_coef) then
   call mo_drag_cpl (thv_atm, thv_surf, z_atm,             &
