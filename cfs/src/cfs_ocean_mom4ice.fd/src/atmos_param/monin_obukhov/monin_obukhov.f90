...
interface mo_drag_cpl
    module procedure  mo_drag_cpl_0d, mo_drag_cpl_1d, mo_drag_cpl_2d
end interface
...
subroutine mo_drag_cpl_1d &
         (pt, pt0, z, z0, zt, zq, speed, fm, ft, fq, &
         drag_m, drag_t, drag_q, u_star, b_star, avail)

real, intent(in)   , dimension(:) :: pt, pt0, z, z0, zt, zq, speed
real, intent(in)   , dimension(:) :: fm, ft, fq
real, intent(inout), dimension(:) :: drag_m, drag_t, drag_q, u_star, b_star
logical, intent(in), optional, dimension(:) :: avail

real   , dimension(size(pt(:))) :: rich, zz
logical, dimension(size(pt(:))) :: mask, mask_1, mask_2
...
do j = 1, size(pt,2)
  call mo_drag_cpl_1d (pt(:,j), pt0(:,j), z(:,j), z0(:,j), zt(:,j), &
                       zq(:,j), speed(:,j), fm(:,j), ft(:,j), fq(:,j), &
                       drag_m(:,j), drag_t(:,j), drag_q(:,j), &
                       u_star(:,j), b_star(:,j))
end do


return
end subroutine mo_drag_cpl_2d
...
subroutine mo_drag_cpl_0d &
    (pt, pt0, z, z0, zt, zq, speed, fm, ft, fq, &
     drag_m, drag_t, drag_q, u_star, b_star)

real, intent(in)    :: z, speed, pt, pt0, z0, zt, zq
real, intent(inout)    :: fm, ft, fq
real, intent(out)   :: drag_m, drag_t, drag_q, u_star, b_star

real, dimension(1) :: pt_1, pt0_1, z_1, z0_1, zt_1, zq_1, speed_1, &
                      drag_m_1, drag_t_1, drag_q_1, u_star_1, b_star_1
real, dimension(1) :: fm_1, ft_1, fq_1
...
call mo_drag_cpl_1d (pt_1, pt0_1, z_1, z0_1, zt_1, zq_1, speed_1, &
                     fm_1, ft_1, fq_1, &
                     drag_m_1, drag_t_1, drag_q_1, u_star_1, b_star_1)
