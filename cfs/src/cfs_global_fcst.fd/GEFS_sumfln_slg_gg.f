      subroutine GEFS_sumfln_slg_gg(flnev,flnod,lat1s,plnev,plnod,
     x                  nvars,ls_node,latl2,
     x                  workdim,nvarsdim,four_gr,
     x                  ls_nodes,max_ls_nodes,
     x                  lats_nodes,global_lats,
     x                  lats_node,ipt_lats_node,lon_dims,
     x                  lons_lat,londi,latl,nvars_0,
     x                  jcap, len_trie_ls, len_trio_ls,
     x                  ls_dim, ls_max_node,
     x                  mpi_r_mpi, latgd, nodes,
     x                  me,mc_comp)
cc
!#include "f_hpm.h"
cc
...
!     use mpi_def   , only : kind_mpi,mc_comp,mpi_r_mpi
      use mpi_def   , only : kind_mpi
      implicit none
!
      integer jcap, latgd
      integer len_trie_ls,len_trio_ls,ls_dim,ls_max_node,me,nodes,
     &        mpi_r_mpi,mc_comp
cc
...
cc
!$omp parallel do private(j,lat,lmax,nvar,lval)
      do j=1,lats_node
         lat = global_lats(ipt_lats_node-1+j)
