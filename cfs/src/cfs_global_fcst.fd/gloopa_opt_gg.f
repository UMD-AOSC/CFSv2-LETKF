...
!
      if( .not. ndsl ) then
        if(zfirst) then
          allocate (szdrdt(lonf*levh,lats_dim_a))
!         szdrdt = 0.
          do j=1,lats_dim_a
            do i=1,lonf*levh
              szdrdt(i,j) = 0.0
...
!
             CALL countperf(0,10,0.)
!mjr         call gfidi_sig(lon_dim  -2, njeff, lat,
!            call gfidi_sig(lon_dim_a-2, njeff, lat,
             call gfidi_sig(lonf, njeff, lat,
     x                   for_gr_a_2(lon,ksd   ,lan),
     x                   for_gr_a_2(lon,kst   ,lan),
     x                   for_gr_a_2(lon,ksz   ,lan),
     x                   for_gr_a_2(lon,ksu   ,lan),
