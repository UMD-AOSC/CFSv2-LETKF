      subroutine impadje(de,te,qe,xe,ye,ze,am,bm,sv,dt,ue,ve,
     x                   snnp1ev,ndexev,
     x                   ls_node,locl)
      ...
      real(kind=kind_evod) am(levs,levs)
      real(kind=kind_evod) bm(levs,levs)
      real(kind=kind_evod) sv(levs)
...
!
      ...
      do k=1,levs
        ...
        do j=1,levs
          do indev = indev1 , indev2
            ve(indev,1,k)=ve(indev,1,k)+am(k,j)*ye(indev,1,j)
            ve(indev,2,k)=ve(indev,2,k)+am(k,j)*ye(indev,2,j)
          enddo
        enddo
          do indev = indev1 , indev2
            ue(indev,1,k)=xe(indev,1,k)
     &     +dt*snnp1ev(indev)*(ve(indev,1,k)+tor_sig(k)*ze(indev,1))
            ue(indev,2,k)=xe(indev,2,k)
     &     +dt*snnp1ev(indev)*(ve(indev,2,k)+tor_sig(k)*ze(indev,2))
          enddo
      enddo
      do k=1,levs
          do indev = indev1 , indev2
        ...
        do j=1,levs
          do indev = indev1 , indev2
            ve(indev,1,k)=
     &      ve(indev,1,k)+dm205(ndexev(indev)+1,k,j)*ue(indev,1,j)
            ve(indev,2,k)=
     &      ve(indev,2,k)+dm205(ndexev(indev)+1,k,j)*ue(indev,2,j)
          enddo
        enddo
      enddo
      do j=1,levs
        do indev = indev1 , indev2
          qe(indev,1)=qe(indev,1)+dt*sv(j)*ve(indev,1,j)
          qe(indev,2)=qe(indev,2)+dt*sv(j)*ve(indev,2,j)
        enddo
      enddo
        do indev = indev1 , indev2
      ...
      do k=1,levs
        do j=1,levs
          do indev = indev1 , indev2
            te(indev,1,k)=te(indev,1,k)+dt*bm(k,j)*ve(indev,1,j)
            te(indev,2,k)=te(indev,2,k)+dt*bm(k,j)*ve(indev,2,j)
          enddo
        enddo
          do indev = indev1 , indev2
      ...
      subroutine impadjo(do,to,qo,xo,yo,zo,am,bm,sv,dt,uo,vo,
     x                   snnp1od,ndexod,
     x                   ls_node,locl)
...
!
      ...
      do k=1,levs
        ...
        do j=1,levs
          do indod = indod1 , indod2
            vo(indod,1,k)=vo(indod,1,k)+am(k,j)*yo(indod,1,j)
            vo(indod,2,k)=vo(indod,2,k)+am(k,j)*yo(indod,2,j)
          enddo
        enddo
          do indod = indod1 , indod2
            uo(indod,1,k)=xo(indod,1,k)
     &     +dt*snnp1od(indod)*(vo(indod,1,k)+tor_sig(k)*zo(indod,1))
            uo(indod,2,k)=xo(indod,2,k)
     &     +dt*snnp1od(indod)*(vo(indod,2,k)+tor_sig(k)*zo(indod,2))
          enddo
      enddo
      do k=1,levs
          do indod = indod1 , indod2
        ...
        do j=1,levs
          do indod = indod1 , indod2
            vo(indod,1,k)=
     &      vo(indod,1,k)+dm205(ndexod(indod)+1,k,j)*uo(indod,1,j)
            vo(indod,2,k)=
     &      vo(indod,2,k)+dm205(ndexod(indod)+1,k,j)*uo(indod,2,j)
          enddo
        enddo
      enddo
      do j=1,levs
        do indod = indod1 , indod2
          qo(indod,1)=qo(indod,1)+dt*sv(j)*vo(indod,1,j)
          qo(indod,2)=qo(indod,2)+dt*sv(j)*vo(indod,2,j)
        enddo
      enddo
        do indod = indod1 , indod2
      ...
      do k=1,levs
        do j=1,levs
          do indod = indod1 , indod2
            to(indod,1,k)=to(indod,1,k)+dt*bm(k,j)*vo(indod,1,j)
            to(indod,2,k)=to(indod,2,k)+dt*bm(k,j)*vo(indod,2,j)
          enddo
        enddo
          do indod = indod1 , indod2
